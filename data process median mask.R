...
data.variable <- data[gene.variable, cell.name]; dim(data.variable)

# mask non-zero that are less than median
threshold <- median(data.variable[data.variable!=0]); threshold

# mask index matrix
mask <- data.variable<=threshold & data.variable>0; dim(mask)
mask <- as(as.matrix(mask), 'ngTMatrix')
...
writeMM(norm.mask.train, paste('masked dataset median all/', dataset, '_mask_train.mtx', sep = ''))
writeMM(norm.mask.test, paste('masked dataset median all/', dataset, '_mask_test.mtx', sep = ''))
writeMM(mask, paste('masked dataset median all/', dataset, '_mask.mtx', sep = ''))
writeMM(norm, paste('masked dataset median all/', dataset, '_data.mtx', sep = ''))
